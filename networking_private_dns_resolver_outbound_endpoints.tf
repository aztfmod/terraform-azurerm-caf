...
module "private_dns_resolver_outbound_endpoints" {
  ...
  subnet_id = try(
    can(each.value.subnet_id) || can(each.value.vnet.key) == false ? each.value.subnet_id : try(local.combined_objects_virtual_subnets[try(each.value.vnet.lz_key, local.client_config.landingzone_key)][each.value.vnet.subnet_key].id,
