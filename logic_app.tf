...
module "logic_app_standard" {
  ...
  app_service_plan_id        = can(each.value.app_service_plan.id) ? each.value.app_service_plan.id : local.combined_objects_app_service_plans[try(each.value.app_service_plan.lz_key, local.client_config.landingzone_key)][each.value.app_service_plan.key].id 
