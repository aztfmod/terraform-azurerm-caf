...
module "packer_build" {
  ...
  build_resource_group_name = can(each.value.build_resource_group.name) || can(each.value.build_resource_group_name) ? try(each.value.build_resource_group.name, each.value.build_resource_group_name) : local.combined_objects_resource_groups[try(each.value.build_resource_group.lz_key, local.client_config.landingzone_key)][try(each.value.build_resource_group_key, each.value.build_resource_group.key)].name
