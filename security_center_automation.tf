module security_center_automation {
  ...
  eventhub_namespace_connection = try(module.event_hub_namespaces[each.value.eventhub_namespace_key].connection_string, null)
  loganalytics_workspace        = try(module.log_analytics[each.value.loganalytics_workspace_key].name, null)
  settings                      = each.value
