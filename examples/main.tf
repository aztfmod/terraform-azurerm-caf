...
locals {
  ...
  # Merge all remote private_dns objects
  lower_private_dns = {
    for key, networking in try(var.landingzone.lower.networking, {}) : key => merge(data.terraform_remote_state.lower_networking[key].outputs.private_dns)
  }
