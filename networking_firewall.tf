module "azurerm_firewalls" {
  ...
  firewall_policy_id = can(each.value.firewall_policy.id) || ( can(each.value.firewall_policy) && can(each.value.firewall_policy.key) == false ) ? try(each.value.firewall_policy.id, null) : local.combined_objects_azurerm_firewall_policies[try(each.value.firewall_policy.lz_key, local.client_config.landingzone_key)][try(each.value.firewall_policy.key, each.value.firewall_policy_key)].id
}
