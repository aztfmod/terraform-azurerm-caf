...
resource "azurerm_automation_schedule" "automation_schedule" {
  ...
  start_time              = try(var.settings.start_time, null) != null ? try(var.settings.start_time_use_current_timestamp, false) ? format("%sT%s", formatdate("YYYY-MM-DD", timeadd(timestamp(), "24h")), var.settings.start_time) : var.settings.start_time : null
