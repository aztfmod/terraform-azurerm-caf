resource "azurerm_federated_identity_credential" "fed_cred" {
  ...
  resource_group_name = coalesce(try(var.resource_group_name, null), try(var.settings.resource_group.name, null), var.resource_groups[try(var.settings.resource_group.lz_key, var.client_config.landingzone_key)][var.settings.resource_group.key].name)
