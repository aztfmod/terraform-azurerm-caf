    ...
    flatten(
      [
        for key, access_policy_object in var.access_policies : {
          keyvault_key            = key
          user_object_id          = lookup(access_policy_object, "object_id", null) == null ? "" : access_policy_object.object_id == "logged_in_user" ? data.azurerm_client_config.current.object_id : var.logged_user_objectId
