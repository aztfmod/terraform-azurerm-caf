...
module "azuread_apps" {
  ...
  keyvault_id = can(var.keyvault_id) || can(each.value.keyvault.id) ? try(var.keyvault_id, each.value.keyvault.id) : var.keyvaults[try(each.value.keyvault_lz_key, each.value.keyvault.lz_key, var.client_config.landingzone_key)][try(var.keyvault_key, each.value.keyvault.key)].id
