...
resource "null_resource" "set_root_cert" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      KEY_VAULT_SECRET_ID      = can(each.value.keyvault_secret_id) || can(each.value.keyvault.certificate_name) || can(each.value.cert_file) ? try(each.value.keyvault_secret_id, data.azurerm_key_vault_certificate.root_certs[each.key].secret_id, null) : each.value.keyvault.secret_id
    }
  }
}
