...
resource "azurerm_lb" "lb" {
  ...
  dynamic "frontend_ip_configuration" {
    ...
    content {
      ...
      subnet_id                     = coalesce(
        try(var.vnets[try(frontend_ip_configuration.value.lz_key, var.client_config.landingzone_key)][frontend_ip_configuration.value.vnet_key].subnets[frontend_ip_configuration.value.subnet_key].id, frontend_ip_configuration.value.subnet_id, null),
        try(var.virtual_subnets[try(frontend_ip_configuration.value.lz_key, var.client_config.landingzone_key)][frontend_ip_configuration.value.subnet_key].id, null)
      )
