module "keyvault" {
  ...
  resource_id = coalesce(
    try(var.remote_objects.keyvaults[each.value.lz_key][each.key].id, null),
    try(var.remote_objects.keyvaults[var.client_config.landingzone_key][each.key].id, null),
