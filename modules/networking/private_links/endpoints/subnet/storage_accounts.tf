...
module "storage_account_remote" {
  ...
  client_config       = var.client_config
  settings            = each.value
  resource_id         = can(each.value.key) ? var.remote_objects.storage_accounts[each.value.lz_key][each.value.key].id : var.remote_objects.storage_accounts[each.value.lz_key][each.key].id
  subresource_names   = toset(try(each.value.private_service_connection.subresource_names, ["blob"]))
  subnet_id           = var.subnet_id
  private_dns         = var.private_dns
  name                = try(each.value.name, each.key)
