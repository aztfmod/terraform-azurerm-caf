...
module "azure_container_registries_remote" {
  source = "../private_endpoint"
  for_each = {
    for key, value in try(var.private_endpoints.azure_container_registries, {}) : key => value
    if can(value.lz_key)
  ...
  client_config       = var.client_config
  settings            = each.value
  resource_id         = can(each.value.key) ? var.remote_objects.azure_container_registries[each.value.lz_key][each.value.key].id : var.remote_objects.azure_container_registries[each.value.lz_key][each.key].id
  subresource_names   = toset(try(each.value.private_service_connection.subresource_names, ["registry"]))
  subnet_id           = var.subnet_id
  private_dns         = var.private_dns
  name                = try(each.value.name, each.key)
