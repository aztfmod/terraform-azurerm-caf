module "redis_caches" {
  ...
  settings            = each.value
  resource_id         = try(var.remote_objects.redis_caches[each.value.lz_key][each.key].id, var.remote_objects.redis_caches[var.client_config.landingzone_key][each.key].id)
