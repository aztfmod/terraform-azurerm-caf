...
output "firewall_id" {
  ...
  value       = try(var.virtual_hub_config.deploy_firewall, false) ? jsondecode(azurerm_resource_group_template_deployment.arm_template_vhub_firewall.*.output_content).resourceID.value : null
}
