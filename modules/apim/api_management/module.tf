...
resource "azurerm_api_management" "apim" {
  ...
  public_ip_address_id  = can(var.settings.public_ip_id) ? var.settings.public_ip_id : can(var.settings.public_ip.key) ? var.public_ip_addresses[try(var.client_config.landingzone_key, var.settings.public_ip.lz_key)][var.settings.public_ip.key].id : null
