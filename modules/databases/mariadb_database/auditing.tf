...
resource "azurerm_mariadb_server_extended_auditing_policy" "mariadb" {
  count = try(var.settings.extended_auditing_policy, null) == null ? 0 : 1

  server_id                               = var.server_id
  storage_endpoint                        = data.azurerm_storage_account.mariadb_auditing.0.primary_blob_endpoint
  storage_account_access_key              = data.azurerm_storage_account.mariadb_auditing.0.primary_access_key
