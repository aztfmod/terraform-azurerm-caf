...
resource "azurerm_search_service" "search_service" {
  ...
  authentication_failure_mode              = try(var.settings.local_authentication_enabled, true) ? try(var.settings.authentication_failure_mode, null) : null
  ...
  customer_managed_key_enforcement_enabled = try(var.settings.customer_managed_key_enforcement_enabled, false)
  ...
  partition_count = ((lower(var.settings.sku) != "free") && (lower(var.settings.sku) != "basic")) ? try(var.settings.partition_count, 1) : 1
  replica_count   = (lower(var.settings.sku) != "free") ? try(var.settings.replica_count, 1) : 1
}