...
resource "azurerm_eventgrid_domain" "evgd" {
  name                     = azurecaf_name.evgd.result
  ...
  //   // for_each = lookup(var.settings, "inbound_ip_rules", null) == null ? [] : [1]
  ...
  // inbound_ip_rule {
  //   for_each = try(var.settings.inbound_ip_rules[*], {})
  //   // for_each = lookup(var.settings, "inbound_ip_rule", null) == null ? [] : [1]

  //     action   = try(inbound_ip_rule.action, "Allow")
  //     value    = [for ip_rule in ]
  //     ip_mask = inbound_ip_rule.ip_mask
    
  // }

  dynamic "inbound_ip_rule" {
    for_each = try(var.settings.inbound_ip_rule, {})
    content {
      ip_mask = inbound_ip_rule.value.ip_mask
