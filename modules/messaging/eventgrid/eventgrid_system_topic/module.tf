...
resource "azurerm_eventgrid_system_topic" "egst" {
  ...
  source_arm_resource_id = can(var.settings.source.id) ? var.settings.communication_services.id : var.remote_objects.communication_services[try(var.settings.communication_services.lz_key, var.client_config.landingzone_key)][var.settings.source.key].id
  tags                   = local.tags
