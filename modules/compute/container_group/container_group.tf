...
resource "azurerm_container_group" "acg" {
  ...
  subnet_ids = can(var.settings.subnet_ids) == false ? null : try(var.settings.subnet_ids.ids, [
        for key, value in var.settings.subnet_ids : can(value.ids) ? value.ids : var.combined_resources.networking[try(value.lz_key, var.client_config.landingzone_key)][value.vnet_key].subnets[value.subnet_key].id
      ])
