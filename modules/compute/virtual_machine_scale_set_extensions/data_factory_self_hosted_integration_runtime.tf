...
data "azurerm_key_vault_secret" "data_factory_shir_key" {
  for_each = var.extension_name == "data_factory_self_hosted_integration_runtime" && try(var.extension.self_hosted_integration_runtime_auth_key, null) == null ? toset(["enabled"]) : toset([])
  name     = var.extension.vault_settings.secret_name
  key_vault_id = try(
    var.keyvault_id,
    try(var.keyvaults[var.client_config.landingzone_key][var.extension.vault_settings.keyvault_key].id, var.keyvaults[var.extension.vault_settings.lz_key][var.extension.vault_settings.keyvault_key].id)
  )
}

resource "azurerm_virtual_machine_scale_set_extension" "adf_shir" {
  depends_on                   = [data.azurerm_key_vault_secret.data_factory_shir_key]
...
locals {
  ...
  shir_vmss_auth_key     = try(data.azurerm_key_vault_secret.data_factory_shir_key["enabled"].value, "")
