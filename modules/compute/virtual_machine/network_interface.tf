...
resource "azurerm_network_interface" "nic" {
  ...
  dynamic "ip_configuration" {
    ...
    content {
      ...
      subnet_id                     = can(ip_configuration.value.subnet_id) ? ip_configuration.value.subnet_id : coalesce(
        try(var.vnets[try(ip_configuration.value.lz_key, var.client_config.landingzone_key)][ip_configuration.value.vnet_key].subnets[ip_configuration.value.subnet_key].id, null),
        try(var.virtual_subnets[try(ip_configuration.value.lz_key, var.client_config.landingzone_key)][ip_configuration.value.subnet_key].id, null)
      )
