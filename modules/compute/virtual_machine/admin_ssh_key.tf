...
resource "azurerm_key_vault_secret" "ssh_private_key" {
  ...
  name         = try(format("%s-ssh-private-key", data.azurecaf_name.linux_computer_name[each.key].result), format("%s-ssh-private-key", azurecaf_name.legacy_computer_name[each.key].result))
...
resource "azurerm_key_vault_secret" "ssh_public_key_openssh" {
  ...
  name         = try(format("%s-ssh-public-key-openssh", data.azurecaf_name.linux_computer_name[each.key].result), format("%s-ssh-public-key-openssh", azurecaf_name.legacy_computer_name[each.key].result))
