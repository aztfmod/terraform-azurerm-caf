...
resource "azurerm_windows_virtual_machine" "vm" {
  ...
  dynamic "additional_unattend_content" {
    for_each = try(each.value.additional_unattend_content, null) != null ? [1] : []
    
      content {

        content = additional_unattend_content.value.content #, filebase64(format("%s/%s", path.cwd,additional_unattend_content.value.contentfile)))
