...
resource "azurerm_windows_virtual_machine" "vm" {
  ...
  dynamic "additional_unattend_content" {
    for_each = try(each.value.additional_unattend_content,{})
    
      content {

        content = additional_unattend_content.key != "setting" ? [1] : additional_unattend_content.key  == "content" ? additional_unattend_content.value.content : file(format("%s/%s", path.cwd, additional_unattend_content.value.contentfile))
