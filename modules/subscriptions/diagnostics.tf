module diagnostics {
  ...
  resource_id       = var.subscription_key == "logged_in_subscription" ? format("/subscriptions/%s", var.primary_subscription_id) : format("/subscriptions/%s", var.settings.subscription_id)
