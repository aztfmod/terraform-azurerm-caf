...
module "kusto_attached_database_configurations" {
  ...
  database_name       = can(each.value.database_name) ? each.value.database_name : local.combined_objects_kusto_databases[try(each.value.kusto_database.lz_key, local.client_config.landingzone_key)][each.value.kusto_database.key].name
}
