module domain_name_registrations {
  ...
  dns_zone_id         = try(local.combined_object_dns_zones[local.client_config.landingzone_key][each.value.dns_zone.key].id, local.combined_object_dns_zones[each.value.dns_zone.lz_key][each.value.dns_zone.key].id)
