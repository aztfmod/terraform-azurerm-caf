module "container_groups" {
  ...
  base_tags            = try(local.global_settings.inherit_tags, false) ? local.resource_groups[try(each.value.lz_key, local.client_config.landingzone_key)][each.value.resource_group_key].tags : {}
  ...
  location            = lookup(each.value, "region", null) == null ? local.resource_groups[try(each.value.lz_key, local.client_config.landingzone_key)][each.value.resource_group_key].location : local.global_settings.regions[each.value.region]
