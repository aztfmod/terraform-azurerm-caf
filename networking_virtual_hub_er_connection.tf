...
module "virtual_hub_er_gateway_connections" {
  ...
  virtual_network_gateway_id = coalesce(
    try(local.combined_objects_virtual_wans[try(each.value.virtual_wan.lz_key, local.client_config.landingzone_key)][each.value.virtual_wan.key].virtual_hubs[each.value.virtual_wan.virtual_hub_key].er_gateway.id, null),
    try(each.value.express_route_gateway_id, null)
  )

  express_route_gateway_name = coalesce(
    try(local.combined_objects_virtual_wans[try(each.value.virtual_wan.lz_key, local.client_config.landingzone_key)][each.value.virtual_wan.key].virtual_hubs[each.value.virtual_wan.virtual_hub_key].er_gateway.name, null),
