module "storage_containers" {
  ...
  storage_account_name = coalesce(
    try(local.combined_objects_storage_accounts[try(each.value.storage_account.lz_key, local.client_config.landingzone_key)][each.value.storage_account.key].name, null),
