...
locals {
  ...
  client_config = {
    ...
    logged_aad_app_objectId = try(var.logged_aad_app_objectId, data.azurerm_client_config.current.object_id)
    logged_user_objectId    = try(var.logged_user_objectId, data.azurerm_client_config.current.object_id)
  }
