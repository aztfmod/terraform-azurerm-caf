module "recovery_vaults" {
  ...
  key_id = can(each.value.encryption.key_id) ? each.value.encryption.key_id : null # try(local.combined_objects_keyvault_keys[try(each.value.encryption.keyvault_key.lz_key, local.client_config.landingzone_key)][each.value.encryption.keyvault_key.key].id, null)
