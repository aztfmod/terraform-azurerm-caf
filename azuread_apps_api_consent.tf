locals {
  ...
  api_permissions_for_sp = flatten(
    [
      for key, app in var.azuread_api_permissions : [
...
resource "null_resource" "grant_admin_consent" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      principalId   = var.user_type == "user" ? null : module.azuread_applications.aad_apps[each.value.aad_app_key].azuread_service_principal.id
      applicationId = module.azuread_applications.aad_apps[each.value.aad_app_key].azuread_application.application_id
    }
  }
}
