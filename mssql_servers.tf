...
module "mssql_servers" {
  ...
  settings          = each.value
  tags              = try(local.global_settings.inherit_tags, false) ? try(each.value.tags, null) : try(local.combined_objects_resource_groups[try(each.value.resource_group.lz_key, local.client_config.landingzone_key)][try(each.value.resource_group.key, each.value.resource_group_key)].tags, {})
