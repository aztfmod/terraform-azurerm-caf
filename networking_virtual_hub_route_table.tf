...
module "azurerm_virtual_hub_route_table" {
  ...
  resource_ids = {
    ...
    # Interim support - Adding only remote virtual_hub_connection. route tables must be deployed in a different tfstate
    #
    virtual_hub_connection = try(var.remote_objects.virtual_hub_connection, {})
  }
}
