...
module "networking" {
  ...
  ddos_id                           = try(each.value.ddos_id, local.combined_objects_ddos_services[try(each.value.ddos_services_lz_key, local.client_config.landingzone_key)][try(each.value.ddos_services_key, each.value.ddos_services_key)].id, "")
