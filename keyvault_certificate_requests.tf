...
module "keyvault_certificate_requests" {
  ...
  cert_secret_name          = can(each.value.cert_secret_name) ? each.value.cert_secret_name : var.security.dynamic_keyvault_secrets[each.value.keyvault_key][each.value.cert_password_key].secret_name
